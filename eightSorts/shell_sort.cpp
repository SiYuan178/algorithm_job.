...
void shell_sort(void* arr, int sz, int width, int (*cmp)(const void* e1, const void* e2))
{
	...
	int i = 0,j=0, inc=0;
	for (inc = sz / 2; inc > 0; inc /= 2)
	{
		for (i = inc; i < sz; i++)
		{

			copy(temp, (char*)arr + i * width, width);
			for (j = i; j >= inc && cmp(temp, (char*)arr + (j - inc) * width) < 0; j = j - inc)
			{
				swap((char*)arr + j * width, (char*)arr + (j - inc) * width, width);
			}
			swap((char*)arr + j * width, temp, width);
		}
	}
	free(temp);
	temp = NULL;
...
int main()
{
	...
	shell_sort(decimal, double_size, sizeof(decimal[0]), double_compar);
	shell_sort(str_arr1, str_size, sizeof(str_arr1[0]), str_compar);
	shell_sort(num, int_size, sizeof(num[0]), int_compar);
